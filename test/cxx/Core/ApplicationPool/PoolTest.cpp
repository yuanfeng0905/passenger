...
namespace tut {
	...
	DEFINE_TEST_GROUP_WITH_LIMIT(Core_ApplicationPool_PoolTest, 110);
	...
	TEST_METHOD(91) {
		...
		while (timer.elapsed() < 500) {
			...
			expectedNumber++;
			ensure_equals(currentSession->getPid(), originalPid);
			currentSession.reset();
			usleep(10000);
		}
		...
		EVENTUALLY(1,
			currentSession = pool->get(options, &ticket);
			result = currentSession->getPid() != originalPid;
			currentSession.reset();
		);
		ensure_equals(pool->getProcessCount(), 1u);
	}
	...
	TEST_METHOD(92) {
		...
		EVENTUALLY(2,
			LockGuard l(pool->syncher);
			result = pool->getGroup("tmp.wsgi")->getWaitlist.size() == 1;
		);
	...
	TEST_METHOD(93) {
		...
		pool->get(options, &ticket);
		debug->debugger->recv("About to attach rolling restarted process");
		ensure_equals(pool->detachGroupByName("tmp.wsgi"), 1u);
		debug->messages->send("Proceed with attaching rolling restarted process");
		EVENTUALLY(2,
			LockGuard l(pool->syncher);
